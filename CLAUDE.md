...
**Version**: 1.25.0 | **Ratified**: 2026-02-17 | **Last Amended**: 2026-07-07
...
- **Inline comments (operator docs)**: Every struct field with `env:"X"` (where X ≠ `"-"`) MUST carry an inline comment on the same line as the field (`// operator-facing description`). This comment is the single source of truth for the docs pipeline — it becomes the description in `docs/reference/configuration.mdx`. Block comments above the field (`field.Doc`) are for Go developers only (section headers, GoDoc). Missing inline comment = pre-commit failure (`ws/scripts/check-config-comments`). Quality: starts with the env var's effect (not Go field name), uses env var names when cross-referencing, one clear sentence, no internal code references.
...
Before designing any new feature or protocol extension, research how established real-time services solve the same problem (Pusher, Ably, Socket.IO, Phoenix Channels, Centrifugo, NATS WebSocket). Document: (1) the common industry pattern, (2) edge cases and failure modes mature implementations handle, (3) where Sukko deviates and why. "Not invented here" solutions to solved problems are forbidden.
...
The Sukko platform spans three sibling repositories that MUST stay in sync whenever this repo adds, removes, or changes APIs, configuration, CLI-facing behavior, or user-visible features. Before any spec is considered complete and before any PR is merged, cross-repo impact MUST be assessed and explicitly documented.

**Sibling repositories** (relative to this repo's parent directory):

- **`../sukko-cli`** — Operator CLI. Impact triggers:
  ...
  - `sukko-cli` has its own `CLAUDE.md` constitution — all CLI changes MUST follow it, not this one
...
- **`../sukko-docs`** — Documentation site. Impact triggers:
  ...
  - User-facing error messages, WebSocket protocol message types, HTTP error codes/error schemas, connection close codes, env var names/defaults/types, CLI command/flag behavior, feature gate additions, or user-visible CLI output format → sukko-docs update required. When a change is user-visible but doesn't match any listed category, assess cross-repo impact explicitly — default is to assess, not skip.
...
1. Every spec (`spec.md`) MUST include a "Docs Requirements" section if any of the above impact triggers apply — listing the exact files and changes needed in `../sukko-docs`. These changes are **in scope** for the feature, not deferred follow-ups.
2. Every spec MUST include a "CLI Requirements" section if any CLI impact triggers apply — listing the changes needed in `../sukko-cli`. If the CLI changes are explicitly deferred, the deferral MUST be stated and tracked, not silently omitted.
...
- **v1.18.0** (2026-05-24): Added XVI — explicit docs/CLI cross-repo impact assessment for every spec and code review.
- **v1.19.0** (2026-05-24): Added XVII — OpenAPI/AsyncAPI specs MUST be updated in the same PR as changes to REST endpoints, schemas, or WebSocket protocol.
...
- **v1.25.0** (2026-07-07): §XIV amended — exception widened from edition-gated data-path E2E to data-path and platform-behavior E2E orchestration (adds the full validate-suite battery); the move-to-Go-binary guidance re-anchored on orchestration complexity rather than scope; file-scope and driver-not-subject invariants unchanged.
