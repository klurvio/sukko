...
func TestNewConsumer_NoBrokers(t *testing.T) {
	...
	cfg := ConsumerConfig{
		...
		Topics:        []string{kafkautil.BuildTopicName("test", "odin", "trade")},
		Logger:        &logger,
		Broadcast:     broadcast,
		ResourceGuard: guard,
	}
...
func TestReplayMessage_Fields(t *testing.T) {
	t.Parallel()
	msg := ReplayMessage{
		Topic:     kafkautil.BuildTopicName("test", "odin", "trade"),
		Partition: 0,
		Offset:    12345,
		Subject:   "BTC.trade",
	...
	if msg.Topic != kafkautil.BuildTopicName("test", "odin", "trade") {
		t.Errorf("Topic = %s, want %s", msg.Topic, kafkautil.BuildTopicName("test", "odin", "trade"))
	}
	if msg.Partition != 0 {
